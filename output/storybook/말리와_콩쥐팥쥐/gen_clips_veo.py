# 말리와 콩쥐팥쥐 - 절정 씬(s04 동물 친구들 도움, s07 꽃신 퐁당) 이미지→영상 클립 생성기
# 폴백 체인: Veo 3.1 Fast → Seedance 1.5(Higgsfield 직접 API) → Veo lite → (compose.py Ken Burns)
# 비용 정책(01_COST_POLICY): Veo 주력, Seedance 보조, Kling 금지. 키/시크릿은 절대 출력하지 않는다.
import base64
import glob
import json
import os
...
CLIP_PROMPTS = {
    "s04": (
        "A lively flock of small brown sparrows flutters and swirls joyfully around the kind Korean girl "
        "in a patched pale-yellow hanbok as they peck and husk rice grains, wings fluttering softly, "
        "while the large gentle black ox slowly pulls the wooden plow across the field behind her, "
        "the girl laughs and spreads her arms happily. "
        f"Joyful, lively, heartwarming. {GHIBLI}"
    ),
    "s07": (
        "The kind Korean girl in a jade-green silk hanbok hops across stepping stones over a clear stream, "
        "one pretty embroidered flower shoe slips off her foot, spins through the air and lands in the water "
        "with a gentle splash and ripple rings, the shoe bobs and drifts away on the current as she reaches "
        "out in surprise. "
        f"Surprising, gentle, storybook climax. {GHIBLI}"
    ),
}
...
def _hf_auth():
    ...
    2026-07-12 검증: 더미 status 조회가 404(=인증 통과)를 반환하는 형식.
    """
