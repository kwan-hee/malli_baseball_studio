# 말리와 우렁각시 - 절정 씬(s04 밥상이 마법처럼 차려짐, s06 우렁이에서 각시가 나와 밥짓기) 이미지→영상 클립 생성기
...
CLIP_PROMPTS = {
    "s04": (
        "Inside a humble cozy Korean cottage kitchen, a warm home-cooked Korean meal magically appears on a low "
        "table by itself: steaming bowls of rice and soup and neat side dishes gently materialize with soft sparkles "
        "of magic light and rising steam, as the young farmer looks around in gentle wonder. "
        f"Cozy, magical, heartwarming. {GHIBLI}"
    ),
    "s06": (
        "Inside a Korean cottage, a graceful young woman in a soft pink hanbok emerges gently from a softly glowing "
        "river snail by a large water jar and begins cooking rice, sparkles of gentle magic drifting in the air, "
        "while a young farmer peeks in secret from behind a door, amazed. "
        f"Wondrous, gentle, magical, tender. {GHIBLI}"
    ),
}
