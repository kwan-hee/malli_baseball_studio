# 말리와 우렁각시 — 썸네일 배경 생성 (좌측 1/3 비움, 글자는 ffmpeg drawtext 후합성)
# 지브리 스타일 고정: STYLE 단일 출처 + 스타일 앵커 첨부 (2026-07-12 필수화)
# 가로(16:9) 강제 — 세로 드리프트 발생 시 재실행 (2026-07-16 교훈)
import base64
import sys
import time
...
PROMPT = (
    ...
    f"YouTube thumbnail background for a children's storybook video: {MALLI} gazing with big curious sparkling eyes "
    "as a graceful young Korean woman in a soft pink hanbok emerges from a softly glowing swirled river snail beside a "
    "large earthen water jar and sets out a warm steaming Korean meal in a cozy cottage kitchen, gentle magic sparkles in the air, wondrous heartwarming mood, "
    "positioned on the RIGHT TWO THIRDS of the frame, bright cheerful colors with strong contrast, "
    "composition leaves the LEFT THIRD of the image relatively simple and soft (plain deep-blue background) for large text overlay, "
    "absolutely NO text, no letters, no words, "
    f"{STYLE}. "
    "WIDE LANDSCAPE 16:9 horizontal orientation, aspect ratio 16:9, resolution 1280x720 (wider than tall)."
)
