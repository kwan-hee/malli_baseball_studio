# 말리와 느림보 달팽이 — 최종 조립: 이미지 Ken Burns + Veo 클립 + 나레이션 + 자막 + BGM → MP4
# 기존 야구사전 compose.py 검증 패턴 이식 (1080p 동화 버전)
import subprocess
from pathlib import Path

from make_narration_v2 import SCENES  # 나레이션 텍스트 단일 출처 (v2: Gemini TTS Kore, 지문 제거 대본)
...
BGM_VOLUME = 0.15
FINAL_FADE = 2.0
SUB_STYLE = "FontName=MemomentKkukkukk,FontSize=20,Outline=2,MarginV=10"  # 자막 바닥 밀착 (2026-07-07 상시 지시, 두 채널 공통)

# Veo 클립 씬 (나머지는 정지 이미지 + Ken Burns)
VIDEO_SCENES = {
    "s07": BASE / "clips" / "s07_veo31_fast_1080p.mp4",
    "s08": BASE / "clips" / "s08_veo31_quality_1080p.mp4",
}
    ...
    if not (out.exists() and out.stat().st_size > 0):
        if sid in VIDEO_SCENES:
            ...
            run([FFMPEG, "-y", "-i", str(img), "-i", str(audio),
                 ...
                 "-c:v", "libx264", "-preset", "ultrafast", "-crf", "20", "-pix_fmt", "yuv420p",
                 "-c:a", "aac", "-b:a", "192k", "-threads", "2", str(out)])
        print(f"{sid}: {sdur:.1f}s {'(veo)' if sid in VIDEO_SCENES else '(kenburns)'}")
    else:
...
joined = BASE / "_joined_v2.mp4"
run([FFMPEG, "-y", "-f", "concat", "-safe", "0", "-i", str(concat_txt),
     "-c:v", "libx264", "-preset", "ultrafast", "-crf", "20", "-c:a", "aac", "-b:a", "192k", str(joined)])

sub = BASE / "_subbed_v2.mp4"
run([FFMPEG, "-y", "-i", "_joined_v2.mp4",
     "-vf", f"subtitles=subs_v2.srt:fontsdir=.:force_style='{SUB_STYLE}'",
     "-c:v", "libx264", "-preset", "ultrafast", "-crf", "20",
     "-c:a", "copy", "-threads", "2", "_subbed_v2.mp4"], cwd=str(BASE))

final = BASE / "말리와_느림보_달팽이_final_v2.mp4"
...
print(f"FINAL: {final.name} ({final.stat().st_size:,} bytes, {t_cursor:.1f}s)")  # cp949 콘솔이 경로의 한자를 못 찍어서 파일명만 출력
