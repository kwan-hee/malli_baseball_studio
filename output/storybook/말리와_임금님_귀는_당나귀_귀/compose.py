# 말리와 임금님 귀는 당나귀 귀 — 최종 조립: 이미지 Ken Burns + 절정 클립 + 나레이션 + 자막 + BGM → MP4
# 최종 출력 1280x720 (2026-07-15 규칙). 완성본 파일명 앞에 ✅ (2026-07-16 규칙 — 폴더에서 완성본 즉시 구분)
# 자막 타이밍 A안 (2026-07-15 사용자 승인): whisper 단어 타임스탬프 실측 SRT — 씬별 로컬 재앵커링
import json
import subprocess
...
VIDEO_SCENES = {
    "s07": BASE / "clips" / "s07_1080p.mp4",  # 대나무숲이 비밀을 노래
    "s09": BASE / "clips" / "s09_1080p.mp4",  # 임금님이 두건 벗고 귀 드러냄(수용)
}
...
final = BASE / "✅말리와_임금님_귀는_당나귀_귀_final.mp4"  # ✅ 접두 = 폴더에서 완성본 즉시 구분
...
print(f"FINAL: (checkmark)_final.mp4 ({final.stat().st_size:,} bytes, {t_cursor:.1f}s)")  # ✅ 이모지는 cp949 콘솔 인코딩 불가 → print에서 제외
