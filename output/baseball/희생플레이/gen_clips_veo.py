# 희생플레이 - 절정 씬(s01 아웃인데 박수 훅, s07 끝내기 희생플라이) 이미지→영상 클립 생성기
# 폴백 체인: Veo 3.1 Fast → Seedance 1.5(Higgsfield 직접 API) → Veo lite → (compose.py Ken Burns)
# 이번 편은 Higgsfield 잔액 부족 추정(100~150)으로 정책 예외 적용 — Veo 우선 (01_COST_POLICY 예외 조항)
# 키/시크릿은 절대 출력하지 않는다.
import base64
import glob
import json
...
CARTOON = (
    "bright cheerful 2D cartoon sports animation, clean bold outlines, vivid warm colors, "
    "generic stylized faces with no real person likeness, smooth simple motion, "
    "static locked camera with no camera shake, no text or numbers anywhere"
)

CLIP_PROMPTS = {
    "s01": (
        "The cartoon batter jogs back toward the dugout with a calm proud smile after being called out "
        "at first base, the umpire pumps his fist in an out call, teammates lean out of the dugout clapping "
        "and cheering warmly, while a runner stands safely on second base dusting himself off, stadium "
        f"lights twinkling. Paradoxical, warm, curious. {CARTOON}"
    ),
    "s07": (
        "The cartoon center fielder makes the catch very deep near the wall, and at the same moment the "
        "runner from third base sprints down the line and crosses home plate with arms spread wide in joy, "
        "teammates burst out of the dugout celebrating and fireworks bloom over the night stadium stands. "
        f"Climactic, jubilant, walk-off drama. {CARTOON}"
    ),
}
